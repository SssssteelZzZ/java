...
if [ -z "${MODEL}" ]; then
  ...
  echo
  echo "  ${C_BLU}Смотри ВНУТРЬ окна с кодом: там своя строка «IP-адрес и порт».${C_OFF}"
  echo "  ${C_BLU}Порт нужен именно оттуда, а не с главного экрана отладки.${C_OFF}"
  echo

  # Даём 3 попытки: код и порт на телевизоре живут пару минут и меняются,
  # поэтому перезапускать весь скрипт из-за опечатки — лишнее.
  PAIRED=0
  for ATTEMPT in 1 2 3; do
    read -r -p "  Порт сопряжения (из окна с кодом): " PPORT
    read -r -p "  Код сопряжения (6 цифр): " PCODE

    # Защита от самой частой ошибки: порт сопряжения не может совпадать
    # с портом подключения. Если совпал — adb pair зависнет навсегда,
    # потому что на том порту слушает обычная отладка, а не сервис сопряжения.
    if [ "${PPORT}" = "${TV_PORT}" ]; then
      echo "  ${C_RED}Это порт подключения, а не сопряжения — они РАЗНЫЕ.${C_OFF}"
      echo "  ${C_RED}Нужный порт написан внутри окна с 6-значным кодом.${C_OFF}"
      echo
      continue
    fi

    echo "  Сопрягаюсь..."
    # Сторожевой таймер: если adb pair всё же подвиснет (неверный порт,
    # закрытое окно, потеря сети) — убиваем его через 25 секунд.
    # На macOS нет команды timeout, поэтому делаем это вручную через фон.
    ...
    PAIR_RC=$?
    kill ${WATCH_PID} 2>/dev/null   # снимаем сторожа, если сопряжение прошло быстро
    ...
    echo "  На телевизоре ЗАКРОЙ и снова ОТКРОЙ «Подключить устройство"
    echo "  с помощью кода подключения» — код и порт там сменятся."
    echo
  done

  if [ "${PAIRED}" != "1" ]; then
    echo "${C_RED}[!] Сопряжение не удалось. Проверь:${C_OFF}"
    echo "    • порт брал ИЗ ОКНА С КОДОМ (он отличается от ${TV_PORT})"
